...
[role="lead"]
Lo spazio totale gestito riportato per un pool di storage (volume) da NetApp SMI-S Provider può differire dalla dimensione mostrata da altri strumenti di gestione dello storage, come FilerView, perché SMI-S Provider include la WAFL e la riserva di Snapshot. Questa discrepanza è un comportamento previsto e non richiede alcuna azione correttiva.
