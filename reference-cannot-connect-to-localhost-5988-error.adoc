...
[role="lead"]
NetApp SMI-S Provider può non riuscire a connettersi a `localhost:5988` quando le impostazioni di connessione necessarie sono disabilitate o configurate in modo errato, oppure quando il provider ha smesso di rispondere. Verifica le impostazioni di connessione e riavvia il provider per risolvere il problema.
